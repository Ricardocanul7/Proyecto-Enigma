/**
 * EnigmaProject - Graphic Icon Functions
 *
 * All functions related to ASCII art icons
 */
...
void padlock_close_icon(void)
{
	...
	int keydraw[][9] = { /** Each number represents an ASCII value */
	...
	for (coord.Y = 0; coord.Y < 5; coord.Y++) { /**Y axis line jumps*/
		gotoxy(60, 10 + coord.Y);
		for (coord.X = 0; coord.X < 9; coord.X++) { /**X axis Coordinates*/
			if (coord.Y < 2) {
			...
			if (coord.X == 8) { /** if it reaches the last position, jump to continue drawing Y++ */
				printf("\n");
			}
		}
	}
...
void about_icon(void)
{
	...
	for (coord.Y = 0; coord.Y < 7; coord.Y++) { /**Y axis line jumps*/
		gotoxy(60, 9 + coord.Y);
		for (coord.X = 0; coord.X < 9; coord.X++) { /**X axis Coordinates*/
			printf("%c", keydraw[coord.Y][coord.X]);
			if (coord.X == 8) { /** if it reaches the last position, jump to continue drawing Y++ */
