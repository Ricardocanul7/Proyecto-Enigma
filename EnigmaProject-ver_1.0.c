...
void UserBox();             /** recuadro de usuario y logotipo de ENIGMA*/
int CompareSringUser();     /**Compara el usuario que se ingresa del teclado con el de la constante*/ 
void Marco();               /**Diseño del Marco del programa*/
void Menu_Principal();      /**Diseño del Menu interactivo de inicio del programa*/
void EncryptDesign();    
int ingresarTEXT();          /**Estructura para ingresar datos de cadena para cada tipo de encriptado... no sera funcion*/
int tipo_cifrado();
void Animacion_Cargando();      /** Animacion de Cargando(CIFRANDO) */
void Animacion_Descifrando();   /** Animacion Descifrado */
void Acerca_de();               /** Info+Animacion de la seccion Acerca de */
int Abrir_Archivo();            /** abrir texto para descifrar */
int tipo_Descifrado();
void Dibujo_Bye();

/** Algoritmos De Cifrado ***************/
int Cifrado_Simple();                /***/
int Cifrado_Run_Length();            /***/
int Cifrado_XOR();                   /***/
int Cifrado_Vigerene();              /***/
/****************************************/

/** Algoritmos De Descifrado ************/
int Descifrado_Simple();             /***/
int Descifrado_Run_Length();         /***/
int Descifrado_XOR();                /***/
int Descifrado_Vigerene();           /***/
/****************************************/

/** ICONOS *******************************/
void Dibujo_Candado();                /***/
void Dibujo_CandadoOPEN();            /***/
void Dibujo_AcercaDe();               /***/
void Dibujo_Salir();                  /***/
/*****************************************/


void SetColorAndBackground(int ForgC, int BackC){  /** funcion para el color */
...
int main(){
	...
	/*******************Ingresar Usuario*******************************************/
	do{                       /**Menu de inicio USUARIO*/
		SetColorAndBackground(50,0);   /**(COLOR TEXT,COLOR BACKGROUND) */
		Marco();
		UserBox();
		compara_cadena = CompareSringUser();
		if(compara_cadena != 0){
			gotoxy(32,10);
	...
	do{
		...
		if(y == 14){
			SetColorAndBackground(9,0);       /**Color Azul*/
			Dibujo_AcercaDe();
		}
		if(y == 17){
			SetColorAndBackground(4,0);       /**Color Rojo*/
			Dibujo_Salir();
		}
		/*********************************************************/
		SetColorAndBackground(50,0);                  /** establecer color de nuevo porque acabo de limpiar la pantalla*/
		Marco();
		gotoxy(10,8);
		printf("Encriptar");
		gotoxy(17,11);
		...
		if(c==enter){
			SetColorAndBackground(50,0);         /** establecer color Verde de nuevo porque acabo de limpiar la pantalla*/
			if(y==8){                  /** Cifrar */
				system("cls");
				gotoxy(13,13);
				/**printf("Aqui va la cuncion de Encriptar");*/ /** aqui va la funcion de Encriptar*/
				tipo = tipo_cifrado();                     /** tipo seria del 1 al 4          */
				system("cls");
				switch(tipo){
				case 1 : 
					EncryptDesign();
					regresar = Cifrado_Simple();   /** Funcion de cifrado inverso*/
					if(regresar == 1){
						break;
					}
				...
				case 2 : 
					EncryptDesign();
					regresar = Cifrado_XOR();    /** Funcion de cifrado XOR*/
					if(regresar == 1){
						break;
					}
					printf("\n\n\t\t   Pulsa una tecla para regresar al menu");
				...
				case 3 : 
					EncryptDesign();
					regresar = Cifrado_Vigerene();    /** funcion de cifrado vigerene*/
					if(regresar == 1){
						break;
					}
					printf("\n\n\t\t   Pulsa una tecla para regresar al menu");
				...
				case 4 : 
					EncryptDesign();
					regresar = Cifrado_Run_Length(); /** funcion de cifrado run length*/
					if(regresar == 1){
						break;
					}
					printf("\n\n\t\t   Pulsa una tecla para regresar al menu");
			...
			if(y==11){              /** Descifrar */
				...
				tipo = tipo_Descifrado();         /** tipo seria del 1 al 4          */
				system("cls");
				switch(tipo){
				case 1 : 
					/** Funcion de cifrado simple*/
					Animacion_Descifrando();
					system("cls");
					regresar = Descifrado_Simple();
					if(regresar == 1){
				...
				case 4 : 
					...
					Animacion_Descifrando();
					system("cls");
					regresar = Descifrado_Run_Length();  
					if(regresar == 1){
						printf("\n\nPresione una tecla para salir");
						getch();break; 
					}
			...
			if(y==14){
				...
				Acerca_de();                                  /** Animacion e Informacion de los desarrolladores */
				getch();
			}
			if(y==17){
				system("cls");
...
void UserBox(){          /** recuadro donde se escribira el nombre de usuario junto a logo de enigma */
	int x, y;
...
int CompareSringUser(){  /** comparar contraseña de usuario para acceder al menu */
...
void EncryptDesign(){
	...
	for(x = 2; x < 77;x++){          /**Sombreado Superior*/
		for(y = 2; y < 8;y++){
			gotoxy(x,y);
			printf("\xB1");
		}
...
int ingresarTEXT(){   /**Funcion Para leer texto dentro del recuadro de MENSAJE(Esta funcion no se usa, solo es un prototipo para recibir texto en cada funcion de cifrado)*/
	char cadena[910],ch, numcaracters, Regresar_Menu = 0;
	int i=0, y = 9, x = 3;
	
	while(ch != 13)               /** Termina si el usuario presiona enter*/
	{
		if((ch == 8)&&(y >= 9)){         /** 8 es la tecla DEL */
		...
		if(i == 910){                    /** determina si ya llego al limite de caracteres para la matriz*/
...
int tipo_cifrado(){
	...
	if((c == derecha)||(c == izquierda)){      /** DIRECCION */
		if(x < 41){
			x+=21;
		}else{
			x = 20;
	...
	if(c == ESC){     /** si presiona la tecla ESC pasa valor 5 y regresa a menu */
		opcion = 5;
		aux = 1;      /** como queremos regresar a menu aux = 1 es la condicion para salir del while */
	}
	
	if(c == enter){                           /** Lo que pasaria en cada caso  */
		aux = 1;
		if((x == 20)&&(y == 8)){              /** si se presiona la tecla enter*/
			opcion = 1;
		}
		if((x == 20)&&(y == 12)){
			opcion = 2;
...
void Acerca_de(){                          /** Aun en Desarrollo */
	int transicion;
	
	/** dibuja todo el relleno de pantalla*
	for(y = 2;y < 24; y++){
		for(x = 2; x < 77;x++){
			gotoxy(x,y);
			printf("\xB1");
...
void Dibujo_Candado(){                     /** ICON */
	...
	for(y = 0;y < 5;y++){          /**saltos del linea o coordenas Y*/
		gotoxy(60,10+y);
		for(x = 0; x < 9; x++){    /**Coordendas X*/
			if(y < 2){
			...
			if(x == 8){            /** si llega al ultimo lugar brinca para seguir dibujando en Y++ */
				printf("\n");
			}
		}
	}
...
void Dibujo_AcercaDe(){                    /** ICON */
	...
	for(y = 0;y < 7;y++){          /**saltos del linea o coordenas Y*/
		gotoxy(60,9+y);
		for(x = 0; x < 9; x++){    /**Coordendas X*/
			printf("%c",keydraw[y][x]);
			if(x == 8){            /** si llega al ultimo lugar brinca para seguir dibujando en Y++ */
...
int tipo_Descifrado(){                    /** Menu de seleccion de descifrado */
	char c;
	int x = 20, y = 8, aux = 0, opcion;
...
int Cifrado_Simple(){                     /** Algoritmo de Cifrado Simple*/
	...
	/**if(i == 1){
		** como solo dio una vuelta el while quiere decir que se presiono ESC no hace nada y sale */
		...
		while (i <= ichar){                      /**Genera el codigo cifrado*/
	...
	/**if(i == 1){
		** como solo dio una vuelta el while quiere decir que se presiono ESC no hace nada y sale */
		...
		for(i = 0; i < ichar; i++){
			...
			if(cadena[i] == cadena[i+1]){          /** si es igual que su anterior entra */
				Repeticiones++;                    /** numero de veces que se repite     */
			}else{
				if(cadena[i] == cadena[i-1]){         /** si no es igual entra              */
					Cifrado[j] = '#';
					j++;
					Cifrado[j] = caracter;
					j++;
	...
	/**if(i == 1){
		** como solo dio una vuelta el while quiere decir que se presiono ESC no hace nada y sale */
		...
		for(i = 0; i < ichar; i++){
			...
			for(j = 0; j <= 7; j++){                 /******* convierte a binario aunque invertido */
		...
		********************************FIN DE CONVERSION A BINARIO ***************************************/
		...
		printf("\n");
		for(i = 0; i < ichar; i++){                /** validar binario de letras con Clave para cifrar */
			for(j = 0; j < 8; j++){
				if(clave[j] == Almacen_Binario[i][j]){   /** si son iguales se cambia valor por 0*/
					CifradoBin[i][j] = 0;
				}else{                                   /** si son diferentes se cambia valor por 1 */
					CifradoBin[i][j] = 1;
				}
			}
		}
...
void Dibujo_Bye(){
	...
	y = 0;
	for(x = 0; x < 41;x++){  /**Impresion de asteriscos horizontales*/
		gotoxy(17+x,15);
		printf("*");
		gotoxy(58-x,7);
		printf("*");
		
		if((x % 5 == 0)){  /** impresion de asteriscos verticales */
			y++;
			gotoxy(17,6+y);
			printf("*");
			gotoxy(58,16-y);
